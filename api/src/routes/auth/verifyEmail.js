const { getLatestEmailVerification, incrementAttempts, markVerified } = require('../../db/queries/email_verifications');
const { setEmailVerified } = require('../../db/queries/users');
...
module.exports = async (req, res, next) => {
  try {
    ...
    const verification = await getLatestEmailVerification(user.id);

    if (!verification) {
      return res.status(400).json({ error: 'no_pending_verification', error_description: 'No pending email verification. Request a new code.', status: 400 });
    }
    ...
    await logEvent({ event_type: 'user.email.verified', user_id: user.id, ip_address: req.ip });
