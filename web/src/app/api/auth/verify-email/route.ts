...
import { getLatestEmailVerification, incrementAttempts, markVerified } from '@/lib/db/queries/email_verifications';
...
export async function POST(req: NextRequest) {
  try {
    ...
    const verification = await getLatestEmailVerification(session.user_id);
    if (!verification) {
      return NextResponse.json(
    ...
    if (!isValid) {
      ...
      await logEvent({
        event_type: 'auth.email.verify.failed',
    ...
    await setEmailVerified(session.user_id);
